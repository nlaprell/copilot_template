...
**Note on Templates**: Template files are preserved in `core/templates/` for reference and reset purposes. Always update the working files in `aiDocs/` (without `.template` extension), not the template files themselves.

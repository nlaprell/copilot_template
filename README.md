...
- Display available MCP server configurations from `mcpServers/`
...
This will:
- Convert `.eml` files to Markdown using `aiScripts/emailToMd/eml_to_md_converter.py`
...
├── aiDocs/                # AI agent documentation
│   ├── templates/         # Template files for clean reset
│   │   ├── AI.template.md
│   │   ├── DISCOVERY.template.md
│   │   ├── SUMMARY.template.md
│   │   └── TASKS.template.md
│   ├── AI.md              # AI agent instructions and workflows
│   ├── DISCOVERY.md       # Discovery questions about the project
│   ├── SUMMARY.md         # Detailed project context for AI
│   └── TASKS.md           # Outstanding and completed tasks
├── aiScripts/             # Automation scripts for AI workflows
│   └── emailToMd/         # Email to Markdown converter
│       ├── README.md
│       └── eml_to_md_converter.py
├── email/                 # Email organization (created automatically)
│   ├── raw/               # Place .eml files here for processing
│   ├── ai/                # Converted Markdown emails
│   └── processed/         # Archived .eml files after conversion
├── mcpServers/            # MCP server configurations
│   ├── awsCore.json
│   ├── context7.json
│   └── github.json
├── prompts/               # Copilot prompts for workflows
│   ├── ProjectInit.prompt.md      # Initialize AI agent
│   ├── discoverEmail.prompt.md    # Process emails and update docs
│   ├── quickStartProject.prompt.md # Complete initialization workflow
│   └── updateSummary.prompt.md    # Generate project summary
├── scripts/               # Utility scripts
│   └── clean-reset.sh     # Reset project to clean template state
├── init.sh                # Interactive MCP server setup
├── SUMMARY.md             # Human-readable project summary (generated)
└── README.md              # This file
```
...
- **`templates/`**: Template files used for clean reset
  ...
  - Use `./scripts/clean-reset.sh` to reset all aiDocs files to templates
...
- **`emailToMd/`**: Email to Markdown converter
  ...
  - See `aiScripts/emailToMd/README.md` for details
...
2. **Configure MCP servers**: Run `./init.sh`
...
The template includes configurations for several MCP servers in `mcpServers/`:

- **github.json**: GitHub API integration
- **awsCore.json**: AWS Labs core MCP server
- **context7.json**: Context7 MCP server

Run `./init.sh` to select which servers to enable in your workspace.

### Adding New MCP Servers

1. Create a new `.json` file in `mcpServers/`
2. Follow the MCP server configuration format
3. Run `./init.sh` to regenerate `.vscode/mcp.json`

...
│   └── mcp.json           # MCP server configuration (generated by .template/init.sh)
...
├── aiScripts/            # Active automation scripts for AI workflows
...
2. **Configure MCP servers**: Run `./.template/init.sh`
...
Run `./.template/init.sh` to select which servers to enable in your workspace.

### Adding New MCP Servers

1. Create a new `.json` file in `.template/mcpServers/`
2. Follow the MCP server configuration format
3. Run `./.template/init.sh` to regenerate `.vscode/mcp.json`
